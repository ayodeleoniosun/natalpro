...
<div id="main-wrapper">
    ...
    <script type="text/javascript">
        ...
        const the_pusher = new Pusher('5d72b785dda223a8d44b', {
            cluster: 'eu'
        });

        var launch_chat = the_pusher.subscribe('launch_chat');
        var online_nurses_request = the_pusher.subscribe('online_nurses_request');
        var nurse_rated = the_pusher.subscribe('nurse_rated');
        var nurse = localStorage.getItem('nurse');
        var user_receive_private_chat = the_pusher.subscribe('user_receive_private_chat');
        var user_send_private_chat = the_pusher.subscribe('user_send_private_chat');
        
        var nurse_send_chat = the_pusher.subscribe('nurse_send_chat');
        var nurse_receive_chat = the_pusher.subscribe('nurse_receive_chat');
        
        
        //for messages
        user_send_private_chat.bind('admin_private_chat', function (data) {
            ...
            addMessage('private_chat'+data.message_id,message,"current", "scroll_responses");
            document.getElementById('pm_message').value = '';
        });

        user_receive_private_chat.bind('admin_private_chat', function (data) {
            
            message = "<div class='chat-content'>";
            message+= "<div class='box bg-light-info'>"+data.message+"</div>";
            message+= "</div>";
            message+= "<div class='chat-time'>"+data.date+"</div><br/>";
            addMessage('private_chat'+data.message_id,message, null, "scroll_responses");
        });

        
        //for chat with patient

        nurse_send_chat.bind('chat_sent', function (data) {
            ...
            addMessage('chat_messages', message, "current", null)
        });

        nurse_receive_chat.bind('chat_sent', function (data) {
            
            message = "<div class='chat-content'>";
            message+= "<div class='box bg-light-info'>"+data.message+"</div>";
            message+= "</div>";
            message+= "<div class='chat-time'>"+data.date+"</div><br/>";
            addMessage('chat_messages', message, null, null);
        });

        online_nurses_request.bind('online_nurses_request', function (data) {
            $("#modal_title").html("Chat Requests");
            ajaxLoadingRequest('/portal/natal-nurse/popup-nurse-request', '#patient_chat_requests', '', 'GET');
        });

        launch_chat.bind('launch_chat', function (data) {
            var row = document.getElementById("accept_div"+data.chat);
            row.parentNode.removeChild(row);

            if(data.nurse == nurse) {
                $('#chatRequestModal').modal('hide');
                window.location = "/portal/natal-nurse/chats/view/"+data.chat_req;
            }
        });

        nurse_rated.bind('nurse_rated', function (data) {
            if(data.nurse == nurse) {
                var status = document.getElementById('feedback_details');
                status.innerHTML = "Rating &nbsp; - &nbsp;<b>"+data.rating+"/5</b><hr/>";
                status.innerHTML+= "Feedback <br/><b><small style='margin-left:20px'>"+data.feedback+"</small></b><hr/><br/>";
